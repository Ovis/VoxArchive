...
/// </summary>
public sealed class TranscriptionSpeechRegionDetector
{
    ...
    public async Task<IReadOnlyList<SpeechRegion>> DetectAsync(string monoWavePath, CancellationToken cancellationToken)
        => await Task.Run(() => Detect(monoWavePath, cancellationToken), cancellationToken);
    ...
    private static IReadOnlyList<SpeechRegion> Detect(string monoWavePath, CancellationToken cancellationToken)
    {
        ...
        if (reader.WaveFormat.Channels != 1) return [new SpeechRegion(TimeSpan.Zero, reader.TotalTime)];
        ...
        for (var i = 1; i < ranges.Count; i++)
        {
            ...
            var last = merged[^1];
            if (current.StartFrame - last.EndFrame <= mergeGapFrames) merged[^1] = (last.StartFrame, Math.Max(last.EndFrame, current.EndFrame));
