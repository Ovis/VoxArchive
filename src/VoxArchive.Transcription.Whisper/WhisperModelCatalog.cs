...
public static class WhisperModelCatalog
{
    ...
    /// <summary>選択可能なWhisperモデル定義</summary>
    public static IReadOnlyList<TranscriptionModelPackageDefinition> All { get; } =
    [
    ...
    private static TranscriptionModelPackageDefinition Create(
        string modelId,
        string displayName,
        string fileName,
        long size,
        string sha256)
        => new(
            WhisperEngineIdentity.EngineId,
            new TranscriptionModelId(modelId),
            displayName,
            modelId,
            Revision,
            "MIT",
            [new(new Uri(BaseUrl + fileName + "?download=true"), fileName, size, sha256)]);
}
