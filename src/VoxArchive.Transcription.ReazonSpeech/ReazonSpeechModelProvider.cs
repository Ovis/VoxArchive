...
/// <summary>
/// ReazonSpeechモデルのcatalogと物理配置規則を提供する
/// </summary>
public sealed class ReazonSpeechModelProvider : ITranscriptionModelProvider
{
    ...
    /// </summary>
    public ReazonSpeechModelProvider(TranscriptionModelPackageInstaller installer)
        : this(installer, null)
    {
    }

    internal ReazonSpeechModelProvider(TranscriptionModelPackageInstaller installer, string? modelsRootDirectory)
    {
        ...
        _modelsRootDirectory = string.IsNullOrWhiteSpace(modelsRootDirectory)
            ...
            : modelsRootDirectory;
        _definitions = ReazonSpeechModelCatalog.All.ToDictionary(x => x.ModelId.Value, StringComparer.OrdinalIgnoreCase);
    }
    ...
    public IReadOnlyList<TranscriptionModelDescriptor> GetAvailableModels()
        => ReazonSpeechModelCatalog.All.Select(ToDescriptor).ToArray();
    ...
    public TranscriptionModelInspection Inspect(TranscriptionModelId modelId, TranscriptionModelInspectionLevel level)
    {
        ...
        return new TranscriptionModelInspection(
            _installer.Inspect(definition, GetInstallationDirectory(definition), level),
            level);
    }
    ...
    public TranscriptionModelInstallation GetInstallation(TranscriptionModelId modelId)
    {
        ...
        if (!IsReady(modelId))
        {
            throw new InvalidOperationException($"ReazonSpeechモデル '{modelId}' は実行可能な状態ではありません。");
        }
    ...
    private TranscriptionModelPackageDefinition Resolve(TranscriptionModelId modelId)
        => _definitions.TryGetValue(modelId.Value, out var definition)
            ? definition
            : throw new NotSupportedException($"未対応のReazonSpeechモデルです: {modelId}");
