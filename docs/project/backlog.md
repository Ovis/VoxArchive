...
## Next
- [x] Whisper連携設計に基づく実装（設定/サービス/ジョブキュー/UI導線）
- [x] Whisperモデル管理（ダウンロード/削除/環境チェック）
- [x] 録音完了後の自動文字起こし（低優先度）
- [x] 出力形式複数選択（txt/srt/vtt/json）
- [x] ライブラリ画面の雛形と一覧表示
...
- [x] 仕様完了チェックリスト作成と判定基準定義
- [x] 長時間検証セッション初期化スクリプトと運用手順の追加
- [x] 品質ゲートを権限制約環境対応に改善
- [x] 仕様完了証跡の自動判定スクリプトを追加
- [x] 証跡テンプレート自動生成と status: PASS 判定を導入
- [x] オフセット補正設定UIと固定遅延補正を実装
